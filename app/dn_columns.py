...
# Default sheet columns that mirror the Google Sheet structure.
_DEFAULT_SHEET_COLUMNS: List[str] = [
    "dn_number",
    "du_id",
    "status_wh",
    "lsp",
...
def _resolve_sheet_base_columns() -> List[str]:
    configured = [column.strip() for column in settings.dn_sheet_columns if column.strip()]
    if configured:
        seen: set[str] = set()
        ordered: List[str] = []
        ...
        return ordered
    return list(_DEFAULT_SHEET_COLUMNS)


SHEET_BASE_COLUMNS: List[str] = _resolve_sheet_base_columns()
...
def get_sheet_columns() -> List[str]:
    ...
    base_columns = list(SHEET_BASE_COLUMNS)
    dynamic_unique = [
        column for column in _dynamic_columns if column not in base_columns
    ]
    return base_columns + dynamic_unique
